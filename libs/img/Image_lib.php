...
class Image_lib {
	...
	var $image_library		= 'gd2';	// Puede ser:  imagemagick, netpbm, gd, gd2
	var $library_path		= '';
	var $dynamic_output		= FALSE;	// Ya sea para enviar al navegador o escribir en el disco
	var $source_image		= '';
	var $new_image			= '';
	var $width				= '';
	var $height				= '';
	var $width_sinEditar	= '';
	var $height_sinEditar	= '';
	var $filled 			= FALSE;  //Si la imagen va con relleno
	var $quality			= '90';
	var $create_thumb		= FALSE;
	var $thumb_marker		= '_thumb';
	var $maintain_ratio		= TRUE;		// Ya sea para mantener la relación de aspecto al cambiar el tamaño o utilizar los valores duros
	var $master_dim			= 'auto';	// automático , altura o anchura . Determina qué utilizar como la dimensión principal
	var $rotation_angle		= '';
	var $x_axis				= '';
	var	$y_axis				= '';
	...
	public function __construct($props = array())
	{
		...
		//echo "Image Lib Class Initialized";
	}
	...
	function image_process_gd($action = 'resize')
	{
		...
		{
			if (($this->orig_width == $this->width AND $this->orig_height == $this->height) AND !$this->filled)
			{
		...
		if($this->filled){
			$dst_img = $create($this->width_sinEditar,$this->height_sinEditar);
			$fondo = imagecolorallocate($dst_img, 255, 255, 255);     
			imagefill($dst_img, 0, 0, $fondo); 
			if($this->master_dim == "height"){
				$x = ($this->width_sinEditar - $this->width)/2;
			}else{
				$y = ($this->height_sinEditar - $this->height)/2;
			}
		}else{
	...
	function image_reproportion()
	{
		...
		$this->width_sinEditar = $this->width;
		$this->height_sinEditar = $this->height;
